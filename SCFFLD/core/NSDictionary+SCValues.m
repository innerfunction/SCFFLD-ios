...
@implementation NSDictionary (SCValues)
...
- (SCValueType)getValueType:(NSString *)name {
    id value = [self getValue:name];
    if (value == nil)                           return SCValueTypeUndefined;
    // NOTE: Can't reliably detect boolean here, as boolean values are represented using NSNumber.
    if ([value isKindOfClass:[NSNumber class]]) return SCValueTypeNumber;
    if ([value isKindOfClass:[NSString class]]) return SCValueTypeString;
    if ([value isKindOfClass:[NSArray class]])  return SCValueTypeList;
    return SCValueTypeObject;
}
*/
...
- (NSString *)getValueAsString:(NSString *)name defaultValue:(NSString *)defaultValue {
    NSString *value = [SCTypeConversions asString:[self getValue:name]];
...
- (NSNumber *)getValueAsNumber:(NSString *)name defaultValue:(NSNumber *)defaultValue {
    NSNumber *value = [SCTypeConversions asNumber:[self getValue:name]];
    return value == nil ? defaultValue : value;
}
...
- (BOOL)getValueAsBoolean:(NSString *)name defaultValue:(BOOL)defaultValue {
    return [self hasValue:name] ? [SCTypeConversions asBoolean:[self getValue:name]] : defaultValue;
}
...
- (NSDate *)getValueAsDate:(NSString *)name defaultValue:(NSDate *)defaultValue {
    NSDate *value = [SCTypeConversions asDate:[self getValue:name]];
    return value == nil ? defaultValue : value;
}
...
- (NSURL *)getValueAsURL:(NSString *)name {
    return [SCTypeConversions asURL:[self getValue:name]];
}

// Return the named property as data.
- (NSData *)getValueAsData:(NSString *)name {
    return [SCTypeConversions asData:[self getValue:name]];
}

// Return the named property as an image.
- (UIImage *)getValueAsImage:(NSString *)name {
    return [SCTypeConversions asImage:[self getValue:name]];
}
