...
- (BOOL)receiveMessage:(SCMessage *)message sender:(id)sender {
    ...
    if ([message hasName:@"show"] || [message hasName:@"open"]) {
        ...
        if ([maybeView isKindOfClass:[UIViewController class]]) {
            ...
            view = [[SCViewController alloc] initWithView:(UIView *)maybeView];
        }
        // Push the new view.
        if (view) {
            ...
            [SCLogger withTag:@"SCNavigationViewController" error:@"Unable to push view parameter of type %@", [view.class description]];
        }
        return YES;
    }
